module Spree
  module Admin
    class AffiliatesController < ResourceController
      ...
      before_action :layout_options, only: %i[new create edit update]
      before_action :build_or_load_affiliate_commission_rule, only: %i[new edit]
      ...
      def build_or_load_affiliate_commission_rule
        Spree::CommissionRule.all.map do |comm_rule|
          @affiliate.affiliate_commission_rules.find_or_initialize_by(commission_rule_id: comm_rule.id)
        end
      end
    end
  end
