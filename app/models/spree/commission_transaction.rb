module Spree
  class CommissionTransaction < Spree::Base
    belongs_to :affiliate, class_name: 'Spree::Affiliate', optional: false
    belongs_to :commission, class_name: 'Spree::Commission', optional: false, counter_cache: :transactions_count
    belongs_to :commissionable, polymorphic: true, optional: false
    ...
    def assign_commission
      ...
      self.commission = Spree::Commission.find_or_create_by(start_date: start_date, end_date: end_date,
                                                            affiliate_id: affiliate.id)
    end

    def cannot_change_commisson
      if persisted? && commission_id.changed?
        errors.add(:base,
                   Spree.t(:cannot_change_commisson,
                           scope: :commission_transaction))
      end
    end
